...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--workflow-url",
        help=(
            "GitHub Actions workflow URL that produced the native artifacts."
        ),
    )
    parser.add_argument(
        "--component",
...
def main() -> int:
    ...
    if not workflow_url:
        raise RuntimeError("--workflow-url is required for native Codex Unleashed artifacts.")

    workflow_ref = parse_workflow_url(workflow_url)
    print(
        "Downloading native artifacts from workflow "
        f"{workflow_ref.run_id} in {workflow_ref.repo}..."
    )

    with _gha_group(f"Download native artifacts from workflow {workflow_ref.run_id}"):
        with tempfile.TemporaryDirectory(prefix="codex-native-artifacts-") as artifacts_dir_str:
            artifacts_dir = Path(artifacts_dir_str)
            _download_artifacts(workflow_ref, artifacts_dir)
...
def parse_workflow_url(workflow_url: str) -> WorkflowRef:
    ...
    if not run_id.isdigit():
        raise ValueError(f"Workflow run id must be numeric: {run_id}")

    return WorkflowRef(repo=repo, run_id=run_id)
...
def _download_artifacts(workflow_ref: WorkflowRef, dest_dir: Path) -> None:
    cmd = [
        "gh",
        "run",
        "download",
