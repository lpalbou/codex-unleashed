...
///
/// 1.  Load `.env` values from the Codex Unleashed home before creating any threads.
...
/// Load env vars from the Codex Unleashed home `.env` file.
///
...
pub fn prepend_path_entry_for_codex_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    ...
    // Use a Codex-home-scoped temp root to avoid cluttering the top-level directory.
