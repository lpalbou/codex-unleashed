...
#[clap(
    ...
    // `codex-unleashed-x86_64-unknown-linux-musl`, but the help output should
    // always use the generic `codex-unleashed` command name that users run.
    bin_name = "codex-unleashed",
    override_usage = "codex-unleashed [OPTIONS] [PROMPT]\n       codex-unleashed [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | codex-unleashed login --with-api-key`)"
    )]
    with_api_key: bool,
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.use_device_code {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | codex-unleashed login --with-api-key`."
                        );
                        std::process::exit(1);
...
/// Build the final `TuiCli` for a `codex-unleashed resume` invocation.
...
/// Build the final `TuiCli` for a `codex-unleashed fork` invocation.
...
/// Merge flags provided to `codex-unleashed resume`/`codex-unleashed fork` so they take precedence over any
...
mod tests {
    ...
    fn format_exit_messages_includes_resume_hint_without_color() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run codex-unleashed resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_prefers_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run codex-unleashed resume my-thread".to_string(),
            ]
        );
    }
