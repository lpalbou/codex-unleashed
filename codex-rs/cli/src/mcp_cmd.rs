...
/// - `add`    — add a server launcher entry to `~/.codex-unleashed/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(
    override_usage = "codex-unleashed mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)"
)]
pub struct AddArgs {
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let codex_home = find_codex_home().context("failed to resolve Codex Unleashed home")?;
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `codex-unleashed mcp login {name}` to login."
        ),
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!(
            "No MCP servers configured yet. Try `codex-unleashed mcp add my-tool -- my-command`."
        );
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: codex-unleashed mcp remove {}", get_args.name);
