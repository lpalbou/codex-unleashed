...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `~/.codex-unleashed/config.toml` snippet:
    ///
    /// ```toml
    ...
    /// file: CODEX_UNLEASHED_HOME/.credentials.json
    ...
    /// Directory containing all Codex state (defaults to `~/.codex-unleashed`
    /// but can be overridden by the `CODEX_UNLEASHED_HOME` environment
    /// variable; `CODEX_HOME` is still accepted as a legacy override).
    pub codex_home: PathBuf,

    /// Directory where Codex stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where Codex writes log files (defaults to the Codex Unleashed
    /// home `log` subdirectory).
    pub log_dir: PathBuf,

    /// Settings that govern if and what will be written to
    /// `~/.codex-unleashed/history.jsonl`.
    pub history: History,
...
/// Patch Codex Unleashed home `config.toml` project state to set trust level.
/// Use with caution.
...
/// Base config deserialized from ~/.codex-unleashed/config.toml.
...
pub struct ConfigToml {
    ...
    /// Directory where Codex stores the SQLite state DB.
    /// Defaults to `$CODEX_SQLITE_HOME` when set. Otherwise uses the Codex
    /// Unleashed home.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Codex writes log files, for example `codex-tui.log`.
    /// Defaults to the Codex Unleashed home `log` subdirectory.
...
///
/// - If `CODEX_UNLEASHED_HOME` is set, the value must exist and be a directory.
///   The value will be canonicalized and this function will Err otherwise.
/// - If `CODEX_UNLEASHED_HOME` is not set, `CODEX_HOME` is still accepted as a
///   legacy override.
