...
//!
//! The history is stored at `~/.codex-unleashed/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside the Codex Unleashed home.
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // Resolve the Codex Unleashed history file and ensure the parent directory exists.
