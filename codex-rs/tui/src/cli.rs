...
pub struct Cli {
    ...
    // Internal controls set by the top-level `codex-unleashed resume` subcommand.
    // These are not exposed as user flags on the base `codex-unleashed` command.
    #[clap(skip)]
    ...
    /// top-level `codex-unleashed resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `codex-unleashed fork` subcommand.
    // These are not exposed as user flags on the base `codex-unleashed` command.
    #[clap(skip)]
    ...
    /// top-level `codex-unleashed fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
