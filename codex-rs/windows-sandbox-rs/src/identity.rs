...
pub fn require_logon_sandbox_creds(
    ...
    // NOTE: Do not add the Codex Unleashed sandbox directory to `needed_write`; it must remain
    // non-writable by the restricted capability token. The setup helper's `lock_sandbox_dir` is
    // responsible for granting the sandbox group access to this directory without granting the
    // capability SID.
