...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, codex_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to the Codex Unleashed home or anything under its
    // sandbox control directories. These locations contain sandbox control/state and helper
    // binaries and must remain tamper-resistant.
